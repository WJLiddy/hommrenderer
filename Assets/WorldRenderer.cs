...
public class WorldRenderer : MonoBehaviour
{
    ...
    private Dictionary<Vector2Int, GameObject> heroes = new Dictionary<Vector2Int, GameObject>();
    ...
    public void renderHeroes(SimpleJSON.JSONNode node, int playerID)
    {
        foreach (SimpleJSON.JSONNode hero in node["heroes"].AsArray)
        {
            ...
            // need this.
            // heroModel.GetComponentInChildren<MeshRenderer>().material = teamUnitColors[hero[2]["player"]];
            heroes[new Vector2Int(x, y)] = heroModel;
            heroModel.transform.position = gridToWorldPosition(x, y);
        }
    }
